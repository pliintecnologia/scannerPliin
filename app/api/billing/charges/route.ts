...
export async function GET() {
  ...
  const result = await query<{ asaas_subscription_id: string }>(
    `SELECT asaas_subscription_id FROM asaas_subscriptions
  ...
  if (!subscriptionId) return NextResponse.json({ charges: [] });
  try {
