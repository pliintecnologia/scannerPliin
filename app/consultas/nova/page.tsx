        ...
        <div className="heroCard">
          <div className="fieldGroup">
            <div className="fieldTitle">
              ...
              <InfoTip label="link do site">Informe o endereço público da página que deseja analisar.</InfoTip>
            </div>
            <input id="site-url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://exemplo.com" />
          </div>
          <div className="fieldGroup">
            <div className="fieldTitle">
              <label htmlFor="html-file">Arquivo HTML</label>
              <InfoTip label="arquivo HTML">Envie um arquivo .html salvo no computador para analisá-lo sem usar um link.</InfoTip>
            </div>
            <input id="html-file" type="file" accept=".html,text/html" onChange={(e) => void onFile(e.target.files?.[0] ?? null)} />
          </div>
          <details className="advancedBox">
            <summary className="advancedSummary">Opções avançadas</summary>
          ...
          <button onClick={analyze} disabled={loading} aria-busy={loading}>
              ...
              {summary.issues.map((item) => (
                <article key={item.id} className={`issueCard wcagCard wcag-${item.severity}`} data-issue-card>
                  ...
                  <div className="wcagFooter">
                    <details className="issueDetails">
                      <summary>Ver correção e HTML</summary>
