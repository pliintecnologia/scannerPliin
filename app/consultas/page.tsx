...
export default async function ConsultasPage() {
  ...
  return <main className="dashboardShell">
    <header className="appHeader"><BrandLogo /><nav aria-label="Navegação principal"><span>Olá, {user.name.split(" ")[0]}</span><Link href="/assinatura">Assinatura</Link><form action="/api/auth/logout" method="post"><button type="submit" className="ghostButton">Sair</button></form></nav></header>
    <section className="dashboardHero"><div><p className="eyebrow">Área de consultas</p><h1>Seus diagnósticos</h1><p>Acompanhe as análises realizadas pela sua conta.</p></div><Link className="primaryLink" href="/app/nova">Nova consulta</Link></section>
    <section className="listPanel">
      {audits.rows.length ? <div className="auditList">{audits.rows.map((audit) => <article className="auditRow" key={audit.id}>
        ...
        <Link className="auditAction" href={`/app/consultas/${audit.id}`}>Ver análise</Link>
      </article>)}</div> : <div className="emptyState"><h2>Nenhuma consulta ainda</h2><p>Faça sua primeira análise para começar o histórico.</p><Link className="primaryLink" href="/app/nova">Realizar primeira consulta</Link></div>}
    </section>
  </main>;
}
